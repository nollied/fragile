    ...
    def _extract_data_from_states(
        ...
        def get_name_from_states(name):
            try:
                ...
                raise ValueError("Data not present in states: %s" % name) from e
...
class HistoryTree(TrackWalkersId):
    ...
    def reset(self, root_id=None, state=None, **kwargs):
        ...
        if root_id is not None:
            parent_ids = self.get("parent_ids").copy()
            parent_ids[:] = root_id
            self.update(parent_ids=parent_ids)

    def update_tree(self):
        id_walkers = self.get("id_walkers")
        self._tree.update(
            parent_ids=self.get("parent_ids"),
            node_ids=id_walkers,

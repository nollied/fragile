...
class ExportedWalkers(States):
    ...
    def __init__(self, batch_size: int, state_dict: typing.StateDict = None):
        """
        ...
        self.update(id_walkers=tensor.ones(self.n, dtype=dtype.hash_type))
    ...
    def get_best_reward(self, minimize: bool) -> typing.Scalar:
        """
...
class ExportSwarm(SwarmWrapper):
    ...
    def _get_export_index(self) -> typing.Tensor:
    ...
    def _create_export_walkers(self, indexes: typing.Tensor) -> ExportedWalkers:
    ...
    def _clone_to_imported(
        ...
        local_ix: typing.Tensor,
        import_ix: typing.Tensor,
    ...
    def _get_merge_indexes(self, walkers: ExportedWalkers) -> Tuple[typing.Tensor, typing.Tensor]:
        """Get the indexes for selecting the walkers that will be compared in \
        the clone operation."""
        local_ix = random_state.choice(
            tensor.arange(len(self.swarm.walkers)), size=self.n_import, replace=False
        )
        if len(walkers) <= self.n_import:
            import_ix = random_state.choice(
                tensor.arange(len(walkers)), size=self.n_import, replace=False
            )
        else:
            import_ix = random_state.choice(
                tensor.arange(len(walkers)), size=self.n_import, replace=True
            )
        return local_ix, import_ix

    def _cross_fai_iteration(
        self, local_ix: typing.Tensor, import_ix: typing.Tensor, walkers: ExportedWalkers
    ) -> Tuple[typing.Tensor, typing.Tensor]:

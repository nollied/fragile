...
class TestStatesWalkers:
    ...
    def test_update(self, states_walkers):
        ...
        states_walkers.reset()
        test_vals = tensor.arange(states_walkers.n)
...
class TestWalkers:
    ...
    def test_get_best_index(self, walkers):
        ...
        oobs_best_idx = 1
        oobs_rewards = tensor.zeros(walkers.n)
    ...
    def test_alive_compas(self, walkers):
        ...
        assert tensor.all(compas == 3), "Type of end_cond: {} end_cond: {}: alive ix: {}".format(
    ...
    def test_update_clone_probs(self, walkers):
        ...
        walkers.reset()
        walkers.update_clone_probs()
        assert tensor.sum(walkers.states.clone_probs == walkers.states.clone_probs[0]) == walkers.n
    ...
    def test_accumulate_rewards(self, walkers):
        ...
        walkers.states.update(cum_rewards=tensor.zeros(len(walkers)))
        rewards = tensor.arange(len(walkers))
        walkers._accumulate_and_update_rewards(rewards)
        assert (walkers.states.cum_rewards == rewards).all()
        walkers._accumulate_rewards = True
        walkers.states.update(cum_rewards=tensor.ones(len(walkers)))
        rewards = tensor.arange(len(walkers))
        walkers._accumulate_and_update_rewards(rewards)

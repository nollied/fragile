...
import judo
from judo import dtype, hasher, States as JudoStates, tensor
import numpy
...
class States(JudoStates):
    ...
    def __init__(self, ignore_clone: Optional[list, tuple, set] = None, **kwargs):
        self.ignore_clone = ignore_clone or set()
        super(States, self).__init__(**kwargs)
    ...
    def clone(
        ...
        """
        ignore = self.ignore_clone if ignore is None else ignore
        for name in self.keys():
            values = self[name]
            if judo.is_tensor(values) and name not in ignore:
                self[name] = values[indexes]
            elif name not in ignore:
                self[name] = [values[i] for i in indexes]
...
class StatesModel(States):
    ...
    def get_params_dict(self) -> StateDict:
        """Return the parameter dictionary with the attributes common to all Models."""
        params = {
...
class StatesWalkers(States):
    ...
    Attributes:
        id_walkers: Array of integers that uniquely identify a given state. \
    ...
    def get_params_dict(self) -> StateDict:
        ...
        of SwarmState that can handle all the data generated by the :class:`Walkers`.
        """
        params = {
    ...
    def reset(self):
        ...
        self.update(
            ...
            cum_rewards=judo.zeros(self.n, dtype=dtype.float),
            virtual_rewards=judo.ones(self.n, dtype=dtype.float),
            distances=judo.zeros(self.n, dtype=dtype.float),
            clone_probs=judo.zeros(self.n, dtype=dtype.float),
            will_clone=judo.zeros(self.n, dtype=dtype.bool),
            in_bounds=judo.ones(self.n, dtype=dtype.bool),
        )
...
class OneWalker(States):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        Args:
            ...
            id_walker: Hash of the provided State. If None it will be calculated when
    ...
    def __repr__(self) -> str:
        """Display information about the walker."""
        import numpy

        with numpy.printoptions(linewidth=100, threshold=200, edgeitems=9):
            string = (
                f"reward: {self.rewards[0]}\n"
                f"time: {self.times[0]}\n"
                f"observ: {self.observs[0].flatten()}\n"
                f"state: {self.states[0].flatten()}\n"
                f"id: {self.id_walkers[0]}"
            )
            return string

    def get_params_dict(self) -> StateDict:
        """Return a dictionary containing the `param_dict` to build an instance \
        of :class:`SwarmState` that can handle all the data generated by the :class:`Walkers`.
        """
        params = {
            "id_walkers": {"dtype": dtype.hash_type},
            "parent_ids": {"dtype": dtype.hash_type},

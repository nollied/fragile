...
class Function(EnvironmentAPI):
    ...
    def __init__(
        ...
        Args:
            function: Callable that takes a batch of vectors (batched across the first
                dimension of the array) and returns a vector of typing.Scalar.
                This function is applied to a batch of walker observations.
    ...
    def param_dict(self) -> StateDict:
        param_dict = {
            "observs": {"shape": self._observs_shape, "dtype": self._observs_dtype, "clone": True},
    ...
    def make_transitions(self):
        ...
        new_points = actions + self.get("observs") if self._actions_as_perturbations else actions
        rewards = self.function(new_points).flatten()
        oobs = self.calculate_oobs(points=new_points, rewards=rewards)
        self.update(observs=new_points, rewards=rewards, oobs=oobs)
    ...
    def reset(self):
        ...
        new_points = self.sample_bounds(batch_size=n_walkers)
        rewards = self.function(new_points).flatten()
        self.update(observs=new_points, rewards=rewards, oobs=oobs)
